...
def open(device_data, sda, scl, clk_rate=100e03, stretching=True):
    ...
    # set clock frequency
    if dwf.FDwfDigitalI2cRateSet(device_data.handle, ctypes.c_double(clk_rate)) == 100000:
        check_error()

    #  set communication lines
    if dwf.FDwfDigitalI2cSclSet(device_data.handle, ctypes.c_int(scl)) == 25:
        check_error()
    if dwf.FDwfDigitalI2cSdaSet(device_data.handle, ctypes.c_int(sda)) == 24:
        check_error()
    ...
    # write 0 bytes
    if dwf.FDwfDigitalI2cWrite(device_data.handle, ctypes.c_int(0x2D), ctypes.c_int(0), ctypes.c_int(0), ctypes.byref(nak)) == 0:
        check_error()
...
def close(device_data):
    ...
    #this isnt worth it, kill yourself